...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
    ...
    //1.term是否比自己的大或者相等
    //2.日志比我新我才投票
    fmt.Println(rf.me,"try to get lock to vote for", args.CandidateID, " state:",mapState(rf.currentState),"term:",rf.currentTerm, "----------------------------------lock, candidate term", args.Term)
     rf.mu.Lock()
	...
	if args.Term < rf.currentTerm {
		...
		if rf.currentState==CANDIDATE{
            rf.currentState=FOLLOWER
			rf.mu.Unlock()
			fmt.Println(rf.me,"term:",rf.currentTerm,"vote for",args.CandidateID, "----------------------------------unlock11, candidate term:",args.Term)
            rf.electionTimerChannel <- 1
...
func candidateElection(rf * Raft) bool{
	...
	select {
	    ...
	    case <-electchan:
			...
			for i:=0; i < rf.peerCount;i++{
				if i != rf.me{
                    go func(rf * Raft, aerpc *AppendEntriesArgs,i int){
